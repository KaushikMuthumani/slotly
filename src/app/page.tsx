  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg)' }}>
      ...
      <section style={{ textAlign: 'center', padding: '100px 24px 80px', maxWidth: 760, margin: '0 auto' }}>
        <div className="badge badge-warning" style={{ marginBottom: 20, display: 'inline-flex' }}>
        ...
        <p style={{ fontSize: '1.2rem', marginBottom: 40, maxWidth: 560, margin: '0 auto 40px' }}>
          Share one link. Clients book a slot, pay via UPI, and get a GST invoice instantly.
          Built for CAs, Lawyers, and Designers across India.
        </p>
      ...
      <section style={{ padding: '60px 24px', maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
          {[
            { icon: '📅', title: 'One Booking Link', desc: 'Share slotly.in/yourname everywhere. Clients pick a slot from your real availability. Zero back-and-forth on WhatsApp.' },
          ...
          <div className="card" style={{ flex: 1, minWidth: 280, maxWidth: 340, textAlign: 'left' }}>
            ...
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 28 }}>
              {['10 bookings/month', '1 service type', 'Email confirmations', 'Basic booking page'].map(f => (
                <li key={f} style={{ fontSize: '0.9375rem', color: 'var(--color-text-secondary)', display: 'flex', gap: 8 }}>
      ...
      <footer style={{ textAlign: 'center', padding: '40px 24px', color: 'var(--color-text-muted)', fontSize: '0.875rem', borderTop: '1px solid var(--color-border)' }}>
        <p>© 2024 Slotly. Made with ❤️ in India.</p>
      </footer>
    </div>
  )
}
