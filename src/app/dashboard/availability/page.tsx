...
export default function AvailabilityPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: blocked } = await supabase.from('blocked_dates').select('*').eq('user_id', user.id).gte('blocked_date', new Date().toISOString().split('T')[0]).order('blocked_date', { ascending: true })
  ...
  const handleSave = async () => {
    ...
    setSaving(false)
    if (insertError) { setError(insertError.message) } else { setSuccess(true); setTimeout(() => setSuccess(false), 3000) }
  }

  const addBlocked = async () => {
    if (!newDate) return
    const { data, error: e } = await supabase.from('blocked_dates').insert({ user_id: userId, blocked_date: newDate, reason: newReason || null }).select().single()
    if (!e && data) { setBlockedDates(prev => [...prev, data].sort((a, b) => a.blocked_date.localeCompare(b.blocked_date))); setNewDate(''); setNewReason('') }
  }

  const removeBlocked = async (id: string) => { await supabase.from('blocked_dates').delete().eq('id', id); setBlockedDates(prev => prev.filter(b => b.id !== id)) }
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, alignItems: 'start' }}>
        ...
        <div className="card">
          ...
          <p style={{ fontSize: '0.875rem', marginBottom: 16 }}>Holidays, leaves, or any date you won't be available.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
            <div className="form-group"><label>Date to Block</label><input type="date" value={newDate} onChange={e => setNewDate(e.target.value)} min={new Date().toISOString().split('T')[0]} /></div>
            <div className="form-group"><label>Reason <span className="label-optional">(optional)</span></label><input type="text" placeholder="Holiday, leave, etc." value={newReason} onChange={e => setNewReason(e.target.value)} /></div>
            <button onClick={addBlocked} className="btn btn-outline btn-full" disabled={!newDate}>+ Block This Date</button>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {blockedDates.map((bd: any) => (
                <div key={bd.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: 'var(--color-bg-muted)', borderRadius: 'var(--radius-md)' }}>
                  <div>
                    <p style={{ fontWeight: 500, fontSize: '0.9375rem' }}>{new Date(bd.blocked_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' })}</p>
